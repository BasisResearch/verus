...
fn late_package_arg_after_irrelevant_arg() {
    ...
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--package=foo"];
...
fn late_features_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--features=some-feature"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_all_features_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--all-features"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_no_default_features_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--no-default-features"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_workspace_arg_after_irrelevant_arg() {
    ...
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--workspace"];
...
fn late_frozen_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--frozen"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_locked_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--locked"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_offline_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--offline"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_target_dir_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--target-dir=/tmp/foo"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_config_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "--config=build.jobs=1"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn late_z_flag_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--verus-irrelevant-arg", "-Z", "unstable-options"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn z_flag_without_space_is_rejected() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "-Zunstable-options"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn z_flag_with_space_is_accepted() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "-Z", "unstable-options"];
    let result = plan_execution(package_dir.path(), args);
...
fn package_before_release_is_ok() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "verify", "--package=foo", "--release"];
...
fn features_before_release_is_ok() {
    ...
    let args = [BIN_NAME, "--mcp", "verify", "--features=default", "--release"];
...
fn focus_late_package_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "focus", "--verus-irrelevant-arg", "--package=foo"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn focus_z_flag_without_space_is_rejected() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "focus", "-Zunstable-options"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn build_late_package_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "build", "--verus-irrelevant-arg", "--package=foo"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn build_z_flag_without_space_is_rejected() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "build", "-Zunstable-options"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn check_late_package_arg_after_irrelevant_arg() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "check", "--verus-irrelevant-arg", "--package=foo"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
...
fn check_z_flag_without_space_is_rejected() {
    let package_dir = MockPackage::new("foo").lib().verify(true).materialize();
    let args = [BIN_NAME, "--mcp", "check", "-Zunstable-options"];
    let result = plan_execution(package_dir.path(), args);
    assert!(result.is_err(), "expected planning to fail for args: {args:?}");
}
