...
//!   read (unprimed or primed), which are guards, and those another selected
//!   invariant calls, which are its helpers;
...
struct Exporter {
    ...
    /// Whether the model is VerusSync's, whose `assert`s are lowered to
    /// `tmp_assert => ...` (see [`Exporter::assert_implication`]).
    verussync: bool,
    ...
    /// The state variables a conjunct-level `v = e` on the unprimed state
    /// has assigned so far in the branch being printed (and the branches
    /// enclosing it): in Init, TLC assigns `x = y` only when `x` is still
...
impl Exporter {
    ...
    fn expr(&mut self, e: &Expr, env: &Env) -> String {
        if self.conj_level {
            if let Some(s) = self.bool_field_assignment(e, env) {
                return s;
            }
        }
    ...
    /// (`s.done`, `!post.done`), printed as the equality it amounts to
    /// (`done = TRUE`, `done' = FALSE`), which TLC can assign from: a bare
    /// `done'` or `~(done)` on an unassigned variable stops it.
    fn bool_field_assignment(&mut self, e: &Expr, env: &Env) -> Option<String> {
        let (field, negated) = bool_field(e)?;
        let value = if negated { "FALSE" } else { "TRUE" };
        if let Some(v) = self.post_field(&field, env) {
            self.current_assigned.insert(v.clone());
            return Some(format!("({v}' = {value})"));
        }
        let v = self.role_field(&field, env, Role::Pre)?;
        self.pre_assigned.insert(v.clone());
        Some(format!("({v} = {value})"))
    }
    ...
    fn branch<T>(&mut self, f: impl FnOnce(&mut Self) -> T) -> (T, BTreeSet<String>) {
        ...
        // A branch sees what was assigned before it, and what it assigns
        // itself does not outlive it.
        let outer_pre = self.pre_assigned.clone();
        let first_call = self.current_calls.len();
        let r = f(self);
        self.pre_assigned = outer_pre;
    ...
    fn orient(&self, a: &Expr, b: &Expr, env: &Env) -> (bool, Option<String>) {
        match (self.post_field(a, env), self.post_field(b, env)) {
            ...
            // With an unprimed field on both sides (`s.x == s.y` in Init),
            // the one already assigned goes on the right.
            (None, None) => {
                match (self.role_field(a, env, Role::Pre), self.role_field(b, env, Role::Pre)) {
                    (None, Some(_)) => (true, None),
                    (Some(x), Some(y)) => {
                        (self.pre_assigned.contains(&x) && !self.pre_assigned.contains(&y), None)
                    }
                    _ => (false, None),
                }
            }
    ...
    fn note_assignment(&mut self, a: &Expr, b: &Expr, env: &Env) {
        ...
        if whole(a, Role::Post) || whole(b, Role::Post) {
            self.current_assigned.extend(self.state_vars.iter().cloned());
            return;
        }
        if whole(a, Role::Pre) || whole(b, Role::Pre) {
            self.pre_assigned.extend(self.state_vars.iter().cloned());
            return;
        }
        if let (_, Some(v)) = self.orient(a, b, env) {
            self.current_assigned.insert(v);
        } else if let Some(v) = self.role_field(a, env, Role::Pre) {
            // `a` is on the left as printed (see [`Exporter::orient`]).
            self.pre_assigned.insert(v);
        }
    }
    ...
    fn is_fieldless(&mut self, inner: &Expr, variant: &str, at_conj: bool, env: &Env) -> String {
        if at_conj {
            if let Some(v) = self.post_field(inner, env) {
                self.current_assigned.insert(v);
            } else if let Some(v) = self.role_field(inner, env, Role::Pre) {
                self.pre_assigned.insert(v);
            }
        }
        let inner_s = self.quiet(|x| x.expr(inner, env));
    ...
    fn pattern(
        ...
        match &p.x {
            ...
            PatternX::Range(lo, hi) => {
                let mut conds = Vec::new();
                if let Some(lo) = lo {
                    let l = self.quiet(|this| this.expr(lo, env));
                    conds.push(format!("({l} <= {subject})"));
                }
                if let Some((hi, op)) = hi {
                    let h = self.quiet(|this| this.expr(hi, env));
                    let sym = if matches!(op, InequalityOp::Lt) { "<" } else { "<=" };
                    conds.push(format!("({subject} {sym} {h})"));
                }
                let cond = match conds.len() {
                    0 => None,
                    1 => conds.pop(),
    ...
    /// bounds `a` by `n - 2`). The bound may not read `unbound` (the binder
    /// itself and the binders after it). The domain is intersected with the
    /// range of the binder's type, so a `u8` binder never takes 256; a type
    /// bound supplies a side the guard leaves open.
    fn bound_from_guard(
        &mut self,
        v: &VarIdent,
        typ: &Typ,
        ...
        let range = match &*crate::ast_util::undecorate_typ(typ) {
        ...
        let mut note = |this: &mut Self, op: InequalityOp, other: &Expr, strict: usize| {
            ...
            match op {
                InequalityOp::Ge | InequalityOp::Gt => lower = Some(shifted("+")),
                InequalityOp::Le | InequalityOp::Lt => upper = Some(shifted("-")),
            }
        };
        ...
        let is_strict = |op: &InequalityOp| matches!(op, InequalityOp::Lt | InequalityOp::Gt);
        let upward = |op: &InequalityOp| matches!(op, InequalityOp::Lt | InequalityOp::Le);
        for g in guard {
            // The comparisons as a chain `args[0] ops[0] args[1] ...`, with
            // `None` for an equality link, which ends a walk.
            let (ops, args): (Vec<Option<InequalityOp>>, Vec<Expr>) = match &g.x {
                ExprX::Binary(BinaryOp::Inequality(op), a, b) => {
                    (vec![Some(*op)], vec![a.clone(), b.clone()])
                }
                ExprX::Multi(MultiOp::Chained(ops), args) => (
                    ops.iter()
                        .map(|op| match op {
                            ChainedOp::Inequality(op) => Some(*op),
                            ChainedOp::MultiEq => None,
                        })
                        .collect(),
            ...
            for i in (0..args.len()).filter(|&i| is_v(&args[i])) {
                // Forward: `v op args[i+1] op ... op args[k]`, all in one
                // direction, up to the first operand the bound may read.
                let (mut dir, mut strict) = (None, 0);
                for k in i + 1..args.len() {
                    let Some(op) = ops[k - 1] else { break };
                    if dir.is_some_and(|d| d != upward(&op)) {
                        break;
                    }
                    dir = Some(upward(&op));
                    if free(&args[k]) {
                        note(self, op, &args[k], strict + is_strict(&op) as usize);
                        break;
                    }
                    strict += is_strict(&op) as usize;
                }
                // Backward: `args[k] op ... op v`, read as `v flip(op) args[k]`.
                let (mut dir, mut strict) = (None, 0);
                for k in (0..i).rev() {
                    let Some(op) = ops[k] else { break };
                    if dir.is_some_and(|d| d != upward(&op)) {
                        break;
                    }
                    dir = Some(upward(&op));
                    if free(&args[k]) {
                        note(self, flip(&op), &args[k], strict + is_strict(&op) as usize);
                        break;
                    }
                    strict += is_strict(&op) as usize;
                }
        ...
        // Intersect with the type's range; it closes a side the guard
        // leaves open.
        let (type_lo, type_hi) = range.as_ref().map(int_type_bounds).unwrap_or((None, None));
        let lower = match (lower, type_lo) {
            (Some(l), Some(t)) => Some(format!("(IF ({l}) > {t} THEN ({l}) ELSE {t})")),
            (l, t) => l.or(t.map(|t| t.to_string())),
        };
        let upper = match (upper, type_hi) {
            (Some(u), Some(t)) => Some(format!("(IF ({u}) < {t} THEN ({u}) ELSE {t})")),
        ...
        match &e.x {
            ...
            ExprX::Logical(LogicalOp::And, ..) => {
                ...
                for c in conjuncts(&e) {
                    // A conjunct sees what the conjuncts before it assigned.
            ...
            ExprX::Binary(BinaryOp::Eq(_), a, b) | ExprX::BinaryOpr(BinaryOpr::ExtEq(..), a, b) => {
                ...
                match (field(a), field(b)) {
                    ...
                    // `s.x == s.y` assigns whichever is not assigned yet
                    // (printed on the left, see [`Exporter::orient`]).
                    (Some(x), Some(y)) if before.contains(&y) => BTreeSet::from([x]),
                    (Some(x), Some(y)) if before.contains(&x) => BTreeSet::from([y]),
                    _ => BTreeSet::new(),
                }
            }
...
fn int_range_pred(s: &str, range: &IntRange) -> Option<String> {
    match int_type_bounds(range) {
        (None, None) => None,
        (Some(lo), None) => Some(format!("({s} >= {lo})")),
        (None, Some(hi)) => Some(format!("({s} <= {hi})")),
        (Some(lo), Some(hi)) => Some(format!("({lo} <= {s} /\\ {s} <= {hi})")),
    }
}

/// The lower and upper bounds of an integer type that TLC's 32-bit integers
/// can hold (see [`int_range_pred`]); `None` for a side that is unbounded or
/// beyond them.
fn int_type_bounds(range: &IntRange) -> (Option<i64>, Option<i64>) {
    match range {
        IntRange::Int | IntRange::Char | IntRange::ISize => (None, None),
        IntRange::I(bits) if *bits >= 32 => (None, None),
        IntRange::Nat | IntRange::USize => (Some(0), None),
        IntRange::U(bits) if *bits >= 31 => (Some(0), None),
        IntRange::U(bits) => (Some(0), Some((1i64 << bits) - 1)),
        IntRange::I(bits) => {
...
/// A bool-typed field read, bare or under one negation (`s.done`,
/// `!s.done`): the read, and whether it is negated. Only field reads are
/// returned; whose field it is is for the caller to decide.
fn bool_field(e: &Expr) -> Option<(Expr, bool)> {
    let e = peel(e);
    let (inner, negated) = match &e.x {
    ...
    (is_field && is_bool).then_some((inner, negated))
}
...
pub fn export_module(krate: &Krate, arg: &str) -> Result<Export, String> {
    ...
    let mut ex = Exporter {
        ...
        verussync: triple.shape == "verussync",
        uses_euclid: false,
        defs: Vec::new(),
        emitted: HashSet::new(),
    ...
    for c in candidates.iter().filter(|c| !c.included && c.reason.starts_with("reached from")) {
        cfg.push_str(&format!(
            "\\* {} is not checked: init/next read it (see the .tla.json candidates)\n",
            c.function
        ));
    }
    for c in candidates.iter().filter(|c| !c.included && c.reason.starts_with("called by")) {
        cfg.push_str(&format!(
            "\\* {} is not checked on its own: another invariant calls it (see the .tla.json\n\\* candidates)\n",
            c.function
        ));
    }
    if inv_names.is_empty() {
